  ...
  useEffect(() => {
    if (localStorage.getItem("selected")) {
      if (!props.mustSpin) {
        ...
        const wine = localSelect
          .filter(
            (e) =>
              (e.selectedbet == rouletteData[prizeNumber].completeOption) |
              (e.selectedbet == oadEven) |
              (e.selectedbet == redBlack) |
              (e.selectedbet == dozen) |
              (e.selectedbet == half) |
              (e.selectedbet == column)
          )
          .map((filteredarray) => ({
            number: filteredarray.selectedbet,
            coin: filteredarray.coin,
          }));
        setWeen(wine);
      }
    }
  ...
  return (
    <div>
      <Ween
        ...
        ween={ween}
        setWeen={setWeen}
        numberween={rouletteData[prizeNumber].completeOption}
        oadEven={oadEven}
        redBlack={redBlack}
        dozen={dozen}
        half={half}
